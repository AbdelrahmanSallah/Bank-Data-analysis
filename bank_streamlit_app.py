...
# PAGE 2 — KPIs DASHBOARD
    ...
    #  a dataframe of unique value counts
...
# PAGE 3 — VISUAL ANALYSIS
    ...
    st.plotly_chart(px.histogram(df, x="age", nbins=25, title="Age Distribution"), use_container_width=True)

    # Boxplot of Balance with Marital status
    st.write("### Balance by Marital Status")
    st.plotly_chart(px.box(df, x="marital", y="balance", title="Balance vs Marital Status"), use_container_width=True)

    # histogram of Balance with Marital status
    st.write("### Balance by Marital Status")
    st.plotly_chart(px.histogram(df, x="marital", y="balance", title="Balance vs Marital Status"), use_container_width=True)


    # Group by marital status and duration
    # How does the average call duration differ across marital status groups?
    st.write("### Average call duration with Marital status ")
    marital_duration = df.groupby('marital')['duration'].mean().reset_index()
    st.plotly_chart(px.bar(marital_duration,x='marital', y='duration',title='Average Call Duration by Marital Status', color='marital'),use_container_width=True)

    # What is the distribution of education levels among clients based on their loan status?
    st.write("### Relation between Education and loan ")
    edu_balance= df.groupby(['loan','education']).size().reset_index(name='count')
    st.plotly_chart(px.bar(edu_balance, x='education', y='count', color='loan', barmode='group', title='Education Distribution by Loan Status'),use_container_width=True)
    # Bar chart
    st.write("### Job Frequency")
    job_count = df['job'].value_counts().reset_index()
    job_count.columns = ["job", "count"]

    st.plotly_chart(px.bar(job_count, x="job", y="count", title="Job Category Counts"), use_container_width=True)

    # Shows if early/mid/late month results in more attempts 
    st.write('### Campaign per month period')
    st.plotly_chart(px.histogram(df, x="month_contacting_period", y="campaign", title="Campaign Attempts per Month Contact Period"),use_container_width=True)

    # Pie
    st.write("### Loan Distribution")
    st.plotly_chart(px.pie(df, names="loan", title="Loan Status Distribution"), use_container_width=True)
